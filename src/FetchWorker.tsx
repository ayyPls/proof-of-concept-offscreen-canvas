...
import { OffscreenSceneProxy } from "./proxy/OrbitControlsProxy";
...
export const FetchWorker = () => {
  ...
  const SceneProxyRef =
    useRef<OffscreenSceneProxy>(null);
  ...
  useEffect(() => {
    ...
    if (canvas !== null && sceneWorkerRef.current === null) {
      sceneWorkerRef.current = new Worker(
        new URL("./workers/scene.worker.js", import.meta.url),
        { type: "module" }
      );
    ...
    if (workerRef.current === null) {
      workerRef.current = new Worker(
        new URL("./workers/fetch.worker.js", import.meta.url), {type: 'module'}
      );
